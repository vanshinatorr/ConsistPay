...
import { Link2, AlertCircle, ExternalLink, Plus } from 'lucide-react';
...
export function PlatformsWidget({ onLinkageChanged, onboardingComplete = true }: PlatformsWidgetProps) {
  ...
  const getPlatformLogo = (platform: string) => {
    switch (platform) {
      ...
      case "Code360":
        return (
          <svg viewBox="15 50 200 365" className="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg">
  ...
  const linkedPlatformsList = (["LeetCode", "GeeksforGeeks"] as const).filter(
    (plat) => !!linkages[plat]
  );

  const unconnectedPlatforms = (["LeetCode", "GeeksforGeeks"] as const).filter(
    (plat) => !linkages[plat]
  );
        ...
        <div className="flex flex-col gap-2 shrink-0">
          ...
          <div className="space-y-0.5">
            {linkedPlatformsList.length > 0 ? (
              ...
              <div className="py-5 border border-dashed border-white/[0.05] rounded-xl flex flex-col items-center justify-center text-center px-4">
                <Link2 className="w-4.5 h-4.5 text-zinc-650 mb-1" />
                <p className="text-[9.5px] text-zinc-500 font-bold">No Profiles Connected</p>
              </div>
            )}
          </div>
        ...
        <div className="mt-2 pt-2 border-t border-white/[0.04] shrink-0">
          ...
          <div className="space-y-0.5">
            {mockPlatforms.filter(p => p.name !== "GitHub").map((mockPlat) => (
              <div key={mockPlat.name} className="flex items-center justify-between py-1 px-0.5 opacity-35 select-none">
                <div className="flex items-center gap-2">
                  <div className="w-5.5 h-5.5 rounded bg-white/5 border border-white/[0.04] flex items-center justify-center shrink-0">
